...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Valuta un LinearSVC sullo split test usando "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Cartella contenente features_test.npy "
            "e metadata_test.parquet."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path del model.joblib salvato durante il training.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Cartella in cui salvare metriche e predizioni.",
    )
...
def load_test_split(features_dir: Path):
    ...
    if unique_labels != [0, 1]:
        raise ValueError(
            "Il test deve contenere entrambe le classi [0, 1]. "
            f"Classi trovate: {unique_labels}"
        )

    if not np.isfinite(X).all():
        raise ValueError(
            "Le feature test contengono NaN oppure Inf."
        )
...
def compute_generator_metrics(
    ...
    """
    Per ogni generatore fake costruisce un test binario:

        tutte le immagini real
        +
        tutte le immagini fake del generatore corrente

    In questo modo AUROC e Average Precision sono calcolabili.
    """
    ...
    if real_rows.empty:
        raise ValueError(
            "Nessuna immagine real disponibile nel test."
        )

    if fake_rows.empty:
        raise ValueError(
            "Nessuna immagine fake disponibile nel test."
        )
...
def main():
    ...
    if (
        ...
        and model_type != "LinearSVC"
    ):
        raise ValueError(
            "Il bundle non contiene un LinearSVC. "
            f"model_type trovato: {model_type}"
        )

    if score_method != "decision_function":
        raise ValueError(
            "Per LinearSVC lo score_method deve essere "
            f"'decision_function'. Trovato: {score_method}"
        )
    ...
    if not hasattr(
        ...
        raise TypeError(
            "Il modello caricato non supporta "
            "decision_function()."
        )

    print(f"  Model type:   {model_type}")
    print(f"  Best C:       {bundle.get('best_c')}")
    print(f"  Best loss:    {bundle.get('best_loss')}")
    print(f"  Normalizer:   {bundle.get('normalization')}")
    ...
    if len(y_score) != len(y_test):
        raise ValueError(
            "Numero di score non coerente con il test: "
            f"scores={len(y_score)}, labels={len(y_test)}"
        )

    if not np.isfinite(y_score).all():
        raise ValueError(
            "Gli score prodotti dal modello contengono "
            "NaN oppure Inf."
        )
    ...
    global_result = {
        ...
        "best_loss": bundle.get(
            "best_loss"
        ),
        "normalization": bundle.get(
            "normalization"
        ),
